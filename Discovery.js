...
const codeServiceOfInterestUuid = '00000001-0002-0003-0004-000000004000' 					//uuid of code service
const unlockCodeCharacteristicOfInterestUuid = '00000001-0002-0003-0004-000000004001' 				//uuid of read/notify characteristic of button A service
const lockCodeCharacteristicOfInterestUuid = '00000001-0002-0003-0004-000000004002' 				//uuid of read/notify characteristic of button A service
...
mqttClient.on('message', messageEventHandler);
	...
	if (services.includes(codeServiceOfInterestUuid)) { //code Services
  	...
  	if (services.includes(buttonServiceOfInterestUuid)) { //Button Services
	 	...
	 	buttonAChar.on('valuechanged', async BtnA => {
	 		...
	 		console.log('Unlock Counter value: ' + parseInt(unlock_counter_val, 10))
	 		
	 		const unlock_code_val = await unlockCodeChar.readValue()
	 		console.log('The Unlock Code is: ' + unlock_code_val.toString('hex'))
	 	})
	...
	if (services.includes(buttonServiceOfInterestUuid)) {
		...
		buttonBChar.on('valuechanged', async BtnB => {
	 		...
	 		console.log('lock Counter value: ' + parseInt(lock_counter_val, 10))
	 		
	 		const lock_code_val = await lockCodeChar.readValue()
	 		console.log('The lock Code is: ' + lock_code_val.toString('hex'))
	 	})
	}
